...
**这些评级还不构成排名。** 目前 <!-- census:begin n-rated -->3000<!-- census:end n-rated --> 个条目有评级，平均每个只比过
...
档位大小因此不均匀，而这种不均匀本身就是结论：<!-- census:begin n-rated -->3000<!-- census:end n-rated --> 个已评级条目中有
<!-- census:begin n-onematch -->1829<!-- census:end n-onematch --> 个只比过 1 场，评级只能落在少数几个离散值上，堆在区间两端。

<!-- census:begin rating-bands -->
| 分档 | 评级区间 | 条目数 | 平均场次 |
| --- | --- | --- | --- |
| 最高档 | 1508–1603 | 1310 | 1.6 |
| 次高档 | 1500–1507 | 294 | 1.8 |
| 次低档 | 1492–1498 | 976 | 1.1 |
| 最低档 | 1427–1489 | 420 | 3.0 |
...
另一半给新条目开张。复配预算的一部分用于**连胜擂台**：选出**比较次数最少**的条目、
按 rating 排序，依次挑战下一位更强的，胜者留在台上继续挑战更强者，因此持续取胜的插件
能在一次运行里连爬数级，而不是赢一场相邻对手就停下。候选取「比较次数最少」而非
「rating 最低」，因为后者会把同一批弱插件永远钉在擂台上——每输一场它们的 rating 仍是最低，
下一轮又被选中。比较与枚举并行执行，两者花的是不同的 API 配额，所以 100 次比较只给
...
**`inconclusive` 有 <!-- census:begin n-inconclusive -->111<!-- census:end n-inconclusive --> 个（<!-- census:begin pct-inconclusive -->0.9%<!-- census:end pct-inconclusive -->），这是扫描本身的局限，不是对那些仓库的判定。**
