...
话题是被完整枚举的，不是采样：<!-- census:begin n-enumerated -->4873<!-- census:end n-enumerated --> 个唯一仓库，做法是围绕搜索 API
单次查询 1000 条的上限做分片。契约探测则跨运行累积，所以下表覆盖的是目前已探测的
<!-- census:begin n-probed -->4498<!-- census:end n-probed --> 个仓库：
...
全部 <!-- census:begin n-probed -->4498<!-- census:end n-probed --> 个被探测仓库，按判定分类：
...
这张表覆盖的是 <!-- census:begin n-enumerated -->4873<!-- census:end n-enumerated --> 个已枚举仓库中不断增长的一部分，而不是每次重新采样。

`VENDORED_HARNESS` 标记的是携带了 harness 副本、而非插件的仓库：它之所以满足
契约，是因为它*内含* DSH 自己的 bundle 包。这 <!-- census:begin n-vendored -->17<!-- census:end n-vendored --> 个的 `fork` 全部为 false，
...
**如实说明的局限：** 第 2、3 级在实践中拦下的很少——<!-- census:begin n-declared -->765<!-- census:end n-declared --> 个声明了 patch
的仓库中只有 <!-- census:begin n-tier23-fail -->15<!-- census:end n-tier23-fail --> 个在这两级失败。静态验证在第 1 级就基本到顶了，剩余的不确定性只能靠实际安装
...
| `high` | 依赖 `@deepseek-ai/dsh-client-*`（client 侧）或 `@deepseek-ai/dsh-host-*` 及 host 专用包（host 侧） | 1834 | 57.3% |
| `declared` | 插件自己的 `dsh.client` 或 `dsh.host` 块声明了该 surface | 765 | 23.9% |
| `medium` | 有 `@deepseek-ai/*` 依赖，但没有一个依赖能区分 client 与 host，surface 记为 `indeterminate` | 132 | 4.1% |
| `low` | 没有 `@deepseek-ai/*` 依赖，surface 由名称或描述中的关键词猜测 | 330 | 10.3% |
| `none` | 既无依赖证据也无关键词命中——**根本没有归因** | 140 | 4.4% |
<!-- census:end surface-zh -->

`high` 与 `medium` 这 <!-- census:begin n-dep-evidence -->1966<!-- census:end n-dep-evidence --> 行基于**已安装的依赖**。另有
<!-- census:begin n-declared -->765<!-- census:end n-declared --> 行为 `declared`：插件自己的 `dsh` 块声明了 surface——这是作者的
...
| `published` | 声明的包名可在 npm registry 解析 | 1597 |
| `git-only` | 不在 npm 上；只能用 Git specifier 安装 | 1521 |
| `unpublishable-scope` | 仓库不属于该组织，却用 `@deepseek-ai/` 命名自己 | 83 |
<!-- census:end install-zh -->

另有 <!-- census:begin n-vendored -->17<!-- census:end n-vendored --> 个仓库原样携带 `@deepseek-ai/dsh-base`（样本外至少还有两个：
...
在 <!-- census:begin n-npm-checked -->1597<!-- census:end n-npm-checked --> 个能在 npm 上解析的包中：

| 状态 | 含义 | 数量 | 占比 |
| --- | --- | --- | --- |
<!-- census:begin npm-manifest-zh -->
| `bundle-ok` | 已发布的清单声明了 `dsh.bundle` | 1499 | 93.9% |
| `bundle-missing` | **已发布的清单没有 `dsh.bundle`**——DSH 会拒绝把它作为 profile bundle 加载 | 91 | 5.7% |
| `package-missing` | 声明的包名已无法在 registry 上解析 | 7 | 0.4% |
| `unreadable` | registry 读取失败；这不是对该包的判断 | 0 | 0.0% |
<!-- census:end npm-manifest-zh -->

其中 <!-- census:begin n-npm-broken -->98<!-- census:end n-npm-broken --> 个（<!-- census:begin pct-npm-broken -->6.1%<!-- census:end pct-npm-broken -->）**按包名装不上**，尽管它们
...
全部 <!-- census:begin n-catalog-rows -->3201<!-- census:end n-catalog-rows --> 个条目：
...
**`inconclusive` 有 <!-- census:begin n-inconclusive -->108<!-- census:end n-inconclusive --> 个（<!-- census:begin pct-inconclusive -->3.9%<!-- census:end pct-inconclusive -->），这是扫描本身的局限，不是对那些仓库的判定。**
...
被判定为失效的 14 个条目（`inconclusive` 不是失效，已排除）：
...
<!-- census:begin n-reviewed -->573<!-- census:end n-reviewed --> 个已评条目，按均值分布：

| 分数 | 含义 | 数量 | 占比 |
| --- | --- | --- | --- |
<!-- census:begin scores-zh -->
| 5 | 扎实、有文档、有测试 | 314 | 54.8% |
| 4 | 可用，文档足以让人采用 | 241 | 42.1% |
| 3 | 一般，实现单薄、缺文档 | 12 | 2.1% |
| 2 | 勉强算插件 | 4 | 0.7% |
| 1 | 空的或坏的 | 2 | 0.3% |
...
**重复采样的结果多数一致。** 被抽到一次以上的 <!-- census:begin n-multi -->42<!-- census:end n-multi --> 个条目中，
<!-- census:begin n-identical -->38<!-- census:end n-identical --> 个两次分数完全相同，<!-- census:begin n-disagree -->4<!-- census:end n-disagree --> 个发生了变化，
...
覆盖率是 <!-- census:begin n-catalogued -->3201<!-- census:end n-catalogued --> 中的 <!-- census:begin n-reviewed -->573<!-- census:end n-reviewed -->，并随每次定时运行增长——每次按小时派生的
...
再按创建日期分片绕过这个上限，最终枚举到 <!-- census:begin n-enumerated -->4873<!-- census:end n-enumerated --> 个唯一仓库——
...
- **公布整个样本的分布**而非精选列表：判定占比、按 star 档的合格率、以及全部
  <!-- census:begin n-probed -->4498<!-- census:end n-probed --> 个被探测仓库的可安装性，探针脚本一并提供。
