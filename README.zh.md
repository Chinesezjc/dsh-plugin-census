...
话题是被完整枚举的，不是采样：<!-- census:begin n-enumerated -->12538<!-- census:end n-enumerated --> 个唯一仓库，做法是围绕搜索 API
单次查询 1000 条的上限做分片。契约探测则跨运行累积，所以下表覆盖的是目前已探测的
<!-- census:begin n-probed -->11498<!-- census:end n-probed --> 个仓库：
...
全部 <!-- census:begin n-probed -->11498<!-- census:end n-probed --> 个被探测仓库，按判定分类：
...
这张表覆盖的是 <!-- census:begin n-enumerated -->12538<!-- census:end n-enumerated --> 个已枚举仓库中不断增长的一部分，而不是每次重新采样。

`VENDORED_HARNESS` 标记的是携带了 harness 副本、而非插件的仓库：它之所以满足
契约，是因为它*内含* DSH 自己的 bundle 包。这 <!-- census:begin n-vendored -->41<!-- census:end n-vendored --> 个的 `fork` 全部为 false，
...
**如实说明的局限：** 第 2、3 级在实践中拦下的很少——<!-- census:begin n-declared -->2064<!-- census:end n-declared --> 个声明了 patch
的仓库中只有 <!-- census:begin n-tier23-fail -->41<!-- census:end n-tier23-fail --> 个在这两级失败。静态验证在第 1 级就基本到顶了，剩余的不确定性只能靠实际安装
...
| `high` | 依赖 `@deepseek-ai/dsh-client-*`（client 侧）或 `@deepseek-ai/dsh-host-*` 及 host 专用包（host 侧） | 5583 | 62.1% |
| `declared` | 插件自己的 `dsh.client` 或 `dsh.host` 块声明了该 surface | 2064 | 22.9% |
| `medium` | 有 `@deepseek-ai/*` 依赖，但没有一个依赖能区分 client 与 host，surface 记为 `indeterminate` | 305 | 3.4% |
| `low` | 没有 `@deepseek-ai/*` 依赖，surface 由名称或描述中的关键词猜测 | 721 | 8.0% |
| `none` | 既无依赖证据也无关键词命中——**根本没有归因** | 322 | 3.6% |
<!-- census:end surface-zh -->

`high` 与 `medium` 这 <!-- census:begin n-dep-evidence -->5888<!-- census:end n-dep-evidence --> 行基于**已安装的依赖**。另有
<!-- census:begin n-declared -->2064<!-- census:end n-declared --> 行为 `declared`：插件自己的 `dsh` 块声明了 surface——这是作者的
...
| `published` | 声明的包名可在 npm registry 解析 | 3980 |
| `git-only` | 不在 npm 上；只能用 Git specifier 安装 | 4858 |
| `unpublishable-scope` | 仓库不属于该组织，却用 `@deepseek-ai/` 命名自己 | 155 |
| `unknown` | **registry 没有给出结论**——这不是对该包的判断 | 2 |
<!-- census:end install-zh -->

另有 <!-- census:begin n-vendored -->41<!-- census:end n-vendored --> 个仓库原样携带 `@deepseek-ai/dsh-base`（样本外至少还有两个：
...
**这些评级还不构成排名。** 目前 <!-- census:begin n-rated -->624<!-- census:end n-rated --> 个条目有评级，平均每个只比过
...
在 <!-- census:begin n-npm-checked -->3980<!-- census:end n-npm-checked --> 个能在 npm 上解析的包中：

| 状态 | 含义 | 数量 | 占比 |
| --- | --- | --- | --- |
<!-- census:begin npm-manifest-zh -->
| `bundle-ok` | 已发布的清单声明了 `dsh.bundle` | 3587 | 90.1% |
| `bundle-missing` | **已发布的清单没有 `dsh.bundle`**——DSH 会拒绝把它作为 profile bundle 加载 | 196 | 4.9% |
| `package-missing` | 声明的包名已无法在 registry 上解析 | 26 | 0.7% |
| `unreadable` | registry 读取失败；这不是对该包的判断 | 171 | 4.3% |
<!-- census:end npm-manifest-zh -->

其中 <!-- census:begin n-npm-broken -->222<!-- census:end n-npm-broken --> 个（<!-- census:begin pct-npm-broken -->5.6%<!-- census:end pct-npm-broken -->）**按包名装不上**，尽管它们
...
全部 <!-- census:begin n-catalog-rows -->8995<!-- census:end n-catalog-rows --> 个条目：
...
<!-- census:begin max-age-days -->15<!-- census:end max-age-days --> 天，所以 30 天的休眠阈值根本还触发不了。这已经不再是过去那种
采样偏差——枚举现在覆盖整个话题，而不是「最近更新的那一页」——但这个数字仍然说明
不了长期维护情况，因为这个生态里还没有任何项目有时间沉寂下来。

**`inconclusive` 有 <!-- census:begin n-inconclusive -->147<!-- census:end n-inconclusive --> 个（<!-- census:begin pct-inconclusive -->1.6%<!-- census:end pct-inconclusive -->），这是扫描本身的局限，不是对那些仓库的判定。**
...
被判定为失效的 44 个条目（`inconclusive` 不是失效，已排除）：
...
覆盖率是 <!-- census:begin n-catalogued -->8995<!-- census:end n-catalogued --> 中的 <!-- census:begin n-reviewed -->1078<!-- census:end n-reviewed -->，并随每次定时运行增长——每次按小时派生的
...
再按创建日期分片绕过这个上限，最终枚举到 <!-- census:begin n-enumerated -->12538<!-- census:end n-enumerated --> 个唯一仓库——
...
- **公布整个样本的分布**而非精选列表：判定占比、按 star 档的合格率、以及全部
  <!-- census:begin n-probed -->11498<!-- census:end n-probed --> 个被探测仓库的可安装性，探针脚本一并提供。
