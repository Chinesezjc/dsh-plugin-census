...
在 999 个仓库的样本上实测（999 是搜索 API 单次查询的上限）：
...
契约，是因为它*内含* DSH 自己的 bundle 包。这 6 个的 `fork` 全部为 false，
...
定时 workflow 直接印证了这一点：在上述数字写下数小时后的一次运行，在同样 999 个
仓库的上限下产出了 742 条而非 763 条目录。两次之间探针没有任何改动——只是这个
话题「最近更新的 999 个仓库」已经是另一批了。
...
**如实说明的局限：** 第 2、3 级在实践中拦下的很少——772 个通过第 1 级的仓库中只有
11 个在这两级失败。静态验证在第 1 级就基本到顶了，剩余的不确定性只能靠实际安装
...
搜索 API 单次查询最多返回 1000 条结果，所以样本是 6081 个中的 999 个唯一仓库
——这个差额是 API 上限造成的，不是筛选造成的。条目按最近更新排序采样。
...
- **vendored harness 检测。** 6 个仓库原样携带 `@deepseek-ai/dsh-base` 且
