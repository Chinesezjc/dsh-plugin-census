...
<!-- census:begin n-reviewed -->93<!-- census:end n-reviewed --> 个已评条目，按均值分布：

| 分数 | 含义 | 数量 | 占比 |
| --- | --- | --- | --- |
<!-- census:begin scores-zh -->
| 5 | 扎实、有文档、有测试 | 52 | 55.9% |
| 4 | 可用，文档足以让人采用 | 38 | 40.9% |
| 3 | 一般，实现单薄、缺文档 | 3 | 3.2% |
| 2 | 勉强算插件 | 0 | 0.0% |
| 1 | 空的或坏的 | 0 | 0.0% |
...
**这些分数会漂移多少，目前还不知道。** 被抽到两次的条目一共两个：一个两次完全相同，
另一个差了 1 分——样本太少，还构不成测量值。漂移数据只在两次抽样撞上同一条目时才增长，而从整个目录里抽两次
