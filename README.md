...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->15895<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->16614<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->16614<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 13340 | 80.3% |
| `NO_DSH_FIELD` | 1343 | 8.1% |
| `NO_PACKAGE_JSON` | 965 | 5.8% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 790 | 4.8% |
...
<!-- census:begin n-enumerated -->15895<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
<!-- census:begin n-declared -->3506<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 7592 | 56.9% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 3506 | 26.3% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 447 | 3.4% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1174 | 8.8% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 621 | 4.7% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->8039<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->3506<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 6149 |
| `git-only` | absent from npm; installable from a Git specifier | 7009 |
...
Over <!-- census:begin n-npm-checked -->6149<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 5533 | 90.0% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 302 | 4.9% |
| `package-missing` | the declared name no longer resolves on the registry | 55 | 0.9% |
| `unreadable` | the registry could not be read; not a statement about the package | 259 | 4.2% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->357<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.8%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->13340<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->40<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
The 2497 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
| `DDDFXYqiming/dsh-side-panel-patched` | archived — repository is archived |
| `ddtcorex/dsh-maestro-devkit` | archived — repository is archived |

...and 2485 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->15895<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->16614<!-- census:end n-probed --> probed repositories, with the probe scripts included.
