...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->16167<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->16968<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->16968<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 13650 | 80.4% |
| `NO_DSH_FIELD` | 1359 | 8.0% |
| `NO_PACKAGE_JSON` | 983 | 5.8% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 800 | 4.7% |
...
<!-- census:begin n-enumerated -->16167<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
<!-- census:begin n-declared -->3629<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 7736 | 56.7% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 3629 | 26.6% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 464 | 3.4% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1190 | 8.7% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 631 | 4.6% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->8200<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->3629<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 6313 |
| `git-only` | absent from npm; installable from a Git specifier | 7151 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 184 |
...
Over <!-- census:begin n-npm-checked -->6313<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 5748 | 91.1% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 312 | 4.9% |
| `package-missing` | the declared name no longer resolves on the registry | 60 | 1.0% |
| `unreadable` | the registry could not be read; not a statement about the package | 193 | 3.1% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->372<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.9%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->13650<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->43<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
The 3556 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 3544 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->16167<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->16968<!-- census:end n-probed --> probed repositories, with the probe scripts included.
