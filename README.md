...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->11498<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->7298<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->7298<!-- census:end n-probed --> probed repositories, by verdict:

| Verdict | Count | Share |
| --- | --- | --- |
<!-- census:begin verdicts -->
| `CONTRACT_OK` | 5307 | 72.7% |
| `NO_DSH_FIELD` | 808 | 11.1% |
| `NO_PACKAGE_JSON` | 600 | 8.2% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 500 | 6.9% |
| `VENDORED_HARNESS` | 30 | 0.4% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 23 | 0.3% |
| `MALFORMED_PACKAGE_JSON` | 14 | 0.2% |
| `PATCH_FILE_MISSING` | 8 | 0.1% |
...
<!-- census:begin n-enumerated -->11498<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
packages. All <!-- census:begin n-vendored -->30<!-- census:end n-vendored --> have `fork: false`, so they are source copies that neither an
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->31<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->1329<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 3021 | 56.9% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 1329 | 25.0% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 216 | 4.1% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 517 | 9.7% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 224 | 4.2% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->3237<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->1329<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| `published` | the declared name resolves on the npm registry | 2673 |
| `git-only` | absent from npm; installable from a Git specifier | 2519 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 113 |
| `unknown` | **the registry did not answer** — not a statement about the package | 2 |
<!-- census:end install-en -->

A further <!-- census:begin n-vendored -->30<!-- census:end n-vendored --> repositories carry `@deepseek-ai/dsh-base` verbatim. They are
...
Over <!-- census:begin n-npm-checked -->2673<!-- census:end n-npm-checked --> packages that resolve on npm:

| State | Meaning | Count | Share |
| --- | --- | --- | --- |
<!-- census:begin npm-manifest -->
| `bundle-ok` | the published manifest declares `dsh.bundle` | 2267 | 84.8% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 140 | 5.2% |
| `package-missing` | the declared name no longer resolves on the registry | 14 | 0.5% |
| `unreadable` | the registry could not be read; not a statement about the package | 252 | 9.4% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->154<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.8%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->5307<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->12<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
The 26 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 14 more in `data/decay.jsonl`.
...
Coverage is <!-- census:begin n-reviewed -->1078<!-- census:end n-reviewed --> of <!-- census:begin n-catalogued -->5307<!-- census:end n-catalogued --> and grows with each scheduled run,
...
<!-- census:begin n-enumerated -->11498<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->7298<!-- census:end n-probed --> probed repositories, with the probe scripts included.
