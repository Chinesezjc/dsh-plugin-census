...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->4873<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->4498<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->4498<!-- census:end n-probed --> probed repositories, by verdict:

| Verdict | Count | Share |
| --- | --- | --- |
<!-- census:begin verdicts -->
| `CONTRACT_OK` | 3201 | 71.2% |
| `NO_DSH_FIELD` | 556 | 12.4% |
| `NO_PACKAGE_JSON` | 401 | 8.9% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 295 | 6.6% |
| `VENDORED_HARNESS` | 17 | 0.4% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 10 | 0.2% |
| `MALFORMED_PACKAGE_JSON` | 7 | 0.2% |
| `PATCH_FILE_MISSING` | 5 | 0.1% |
| `BUNDLE_UNDETERMINED` | 3 | 0.1% |
| `TREE_UNREADABLE` | 2 | 0.0% |
...
<!-- census:begin n-enumerated -->4873<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
packages. All <!-- census:begin n-vendored -->17<!-- census:end n-vendored --> have `fork: false`, so they are source copies that neither an
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->15<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->765<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 1834 | 57.3% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 765 | 23.9% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 132 | 4.1% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 330 | 10.3% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 140 | 4.4% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->1966<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->765<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| `published` | the declared name resolves on the npm registry | 1597 |
| `git-only` | absent from npm; installable from a Git specifier | 1521 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 83 |
<!-- census:end install-en -->

A further <!-- census:begin n-vendored -->17<!-- census:end n-vendored --> repositories carry `@deepseek-ai/dsh-base` verbatim. They are
...
Over <!-- census:begin n-npm-checked -->1597<!-- census:end n-npm-checked --> packages that resolve on npm:

| State | Meaning | Count | Share |
| --- | --- | --- | --- |
<!-- census:begin npm-manifest -->
| `bundle-ok` | the published manifest declares `dsh.bundle` | 1499 | 93.9% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 91 | 5.7% |
| `package-missing` | the declared name no longer resolves on the registry | 7 | 0.4% |
| `unreadable` | the registry could not be read; not a statement about the package | 0 | 0.0% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->98<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->6.1%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->3201<!-- census:end n-catalog-rows --> entries:
...
**`inconclusive` is <!-- census:begin n-inconclusive -->108<!-- census:end n-inconclusive --> entries (<!-- census:begin pct-inconclusive -->3.9%<!-- census:end pct-inconclusive -->), and that is a
...
The 14 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 2 more in `data/decay.jsonl`.
...
Over <!-- census:begin n-reviewed -->573<!-- census:end n-reviewed --> reviewed entries, by mean score:

| Score | Meaning | Count | Share |
| --- | --- | --- | --- |
<!-- census:begin scores-en -->
| 5 | substantial, documented, tested | 314 | 54.8% |
| 4 | solid and usable | 241 | 42.1% |
| 3 | ordinary, thin, undocumented | 12 | 2.1% |
| 2 | barely a plugin | 4 | 0.7% |
...
**Repeated samples agree more often than not.** Of the <!-- census:begin n-multi -->42<!-- census:end n-multi --> entries sampled
more than once, <!-- census:begin n-identical -->38<!-- census:end n-identical --> returned an identical score and
<!-- census:begin n-disagree -->4<!-- census:end n-disagree --> moved, with a mean spread of <!-- census:begin mean-spread -->0.12<!-- census:end mean-spread --> points and a
...
Coverage is <!-- census:begin n-reviewed -->573<!-- census:end n-reviewed --> of <!-- census:begin n-catalogued -->3201<!-- census:end n-catalogued --> and grows with each scheduled run,
...
<!-- census:begin n-enumerated -->4873<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->4498<!-- census:end n-probed --> probed repositories, with the probe scripts included.
