...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->15037<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->15617<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->15617<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 12465 | 79.8% |
| `NO_DSH_FIELD` | 1292 | 8.3% |
| `NO_PACKAGE_JSON` | 924 | 5.9% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 765 | 4.9% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 51 | 0.3% |
...
<!-- census:begin n-enumerated -->15037<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->68<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->3172<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 7217 | 57.9% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 3172 | 25.4% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 412 | 3.3% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1100 | 8.8% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 564 | 4.5% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->7629<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->3172<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 5687 |
| `git-only` | absent from npm; installable from a Git specifier | 6599 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 177 |
...
Over <!-- census:begin n-npm-checked -->5687<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 5023 | 88.3% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 274 | 4.8% |
| `package-missing` | the declared name no longer resolves on the registry | 41 | 0.7% |
| `unreadable` | the registry could not be read; not a statement about the package | 349 | 6.1% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->315<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.5%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->12465<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->35<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
**`inconclusive` is <!-- census:begin n-inconclusive -->88<!-- census:end n-inconclusive --> entries (<!-- census:begin pct-inconclusive -->0.7%<!-- census:end pct-inconclusive -->), and that is a
...
The 406 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 394 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->15037<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->15617<!-- census:end n-probed --> probed repositories, with the probe scripts included.
