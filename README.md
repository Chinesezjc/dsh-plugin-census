...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->15741<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->16447<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->16447<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 13192 | 80.2% |
| `NO_DSH_FIELD` | 1337 | 8.1% |
| `NO_PACKAGE_JSON` | 958 | 5.8% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 784 | 4.8% |
...
<!-- census:begin n-enumerated -->15741<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
<!-- census:begin n-declared -->3465<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 7532 | 57.1% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 3465 | 26.3% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 444 | 3.4% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1157 | 8.8% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 594 | 4.5% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->7976<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->3465<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 6093 |
| `git-only` | absent from npm; installable from a Git specifier | 6918 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 179 |
...
Over <!-- census:begin n-npm-checked -->6093<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 5392 | 88.5% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 299 | 4.9% |
| `package-missing` | the declared name no longer resolves on the registry | 52 | 0.9% |
| `unreadable` | the registry could not be read; not a statement about the package | 350 | 5.7% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->351<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.8%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->13192<!-- census:end n-catalog-rows --> entries:
...
The 2078 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 2066 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->15741<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->16447<!-- census:end n-probed --> probed repositories, with the probe scripts included.
