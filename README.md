...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->16072<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->16842<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->16842<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 13535 | 80.4% |
| `NO_DSH_FIELD` | 1355 | 8.0% |
| `NO_PACKAGE_JSON` | 977 | 5.8% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 799 | 4.7% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 54 | 0.3% |
| `VENDORED_HARNESS` | 49 | 0.3% |
| `TREE_UNREADABLE` | 26 | 0.2% |
| `MALFORMED_PACKAGE_JSON` | 23 | 0.1% |
...
<!-- census:begin n-enumerated -->16072<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
packages. All <!-- census:begin n-vendored -->49<!-- census:end n-vendored --> have `fork: false`, so they are source copies that neither an
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->71<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->3580<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 7685 | 56.8% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 3580 | 26.4% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 459 | 3.4% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1185 | 8.8% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 626 | 4.6% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->8144<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->3580<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 6262 |
| `git-only` | absent from npm; installable from a Git specifier | 7091 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 180 |
| `unknown` | **the registry did not answer** — not a statement about the package | 2 |
<!-- census:end install-en -->

A further <!-- census:begin n-vendored -->49<!-- census:end n-vendored --> repositories carry `@deepseek-ai/dsh-base` verbatim. They are
...
**These ratings are not yet a ranking.** <!-- census:begin n-rated -->4676<!-- census:end n-rated --> entries have a rating,
averaging <!-- census:begin rating-matches-mean -->1.8<!-- census:end rating-matches-mean --> comparisons each with a maximum of
<!-- census:begin rating-matches-max -->41<!-- census:end rating-matches-max -->, and the spread is only
<!-- census:begin rating-spread -->1427 to 1708<!-- census:end rating-spread -->. Elo needs roughly 10 to 20 matches before a rating
...
therefore uneven, and that unevenness is the finding: <!-- census:begin n-onematch -->2096<!-- census:end n-onematch --> of the
<!-- census:begin n-rated -->4676<!-- census:end n-rated --> rated entries have exactly one comparison, so ratings sit on a few discrete
values and pile up at each end of the range.

<!-- census:begin rating-bands -->
| Band | Rating range | Entries | Mean matches |
| --- | --- | --- | --- |
| top band | 1508–1708 | 1841 | 1.8 |
| second band | 1500–1507 | 693 | 1.9 |
| third band | 1492–1498 | 1194 | 1.2 |
| bottom band | 1427–1489 | 948 | 2.4 |
...
collapsed into a single row — the most-compared entries sit there, because repeated
comparisons move a rating off the few values one match can produce.
...
Over <!-- census:begin n-npm-checked -->6262<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 5639 | 90.1% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 308 | 4.9% |
| `package-missing` | the declared name no longer resolves on the registry | 59 | 0.9% |
| `unreadable` | the registry could not be read; not a statement about the package | 256 | 4.1% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->367<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.9%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->13535<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->42<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
**`inconclusive` is <!-- census:begin n-inconclusive -->4<!-- census:end n-inconclusive --> entries (<!-- census:begin pct-inconclusive -->0.0%<!-- census:end pct-inconclusive -->), and that is a
...
The 3176 entries flagged as decayed (`inconclusive` is not decay and is excluded):

| Entry | State |
| --- | --- |
| `1HelloMan1/dsh-stats-dashboard` | archived — repository is archived |
| `777-Zen/dsh-capability-index` | archived — repository is archived |
| `an4nsi/dsh-fork-view` | archived — repository is archived |
| `Andy8647/dsh-automode` | archived — repository is archived |
...
| `careyourcake/dsh-plugin-litsearch-zotero` | archived — repository is archived |
...
...and 3164 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->16072<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->16842<!-- census:end n-probed --> probed repositories, with the probe scripts included.
