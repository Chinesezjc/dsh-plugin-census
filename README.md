...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->12378<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->10798<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->10798<!-- census:end n-probed --> probed repositories, by verdict:

| Verdict | Count | Share |
| --- | --- | --- |
<!-- census:begin verdicts -->
| `CONTRACT_OK` | 8398 | 77.8% |
| `NO_DSH_FIELD` | 980 | 9.1% |
| `NO_PACKAGE_JSON` | 713 | 6.6% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 599 | 5.5% |
| `VENDORED_HARNESS` | 37 | 0.3% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 26 | 0.2% |
| `MALFORMED_PACKAGE_JSON` | 16 | 0.1% |
| `TREE_UNREADABLE` | 12 | 0.1% |
| `PATCH_FILE_MISSING` | 11 | 0.1% |
...
<!-- census:begin n-enumerated -->12378<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
packages. All <!-- census:begin n-vendored -->37<!-- census:end n-vendored --> have `fork: false`, so they are source copies that neither an
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->37<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->1889<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 5252 | 62.5% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 1889 | 22.5% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 287 | 3.4% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 668 | 8.0% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 302 | 3.6% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->5539<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->1889<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| `published` | the declared name resolves on the npm registry | 3663 |
| `git-only` | absent from npm; installable from a Git specifier | 4584 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 149 |
| `unknown` | **the registry did not answer** — not a statement about the package | 2 |
<!-- census:end install-en -->

A further <!-- census:begin n-vendored -->37<!-- census:end n-vendored --> repositories carry `@deepseek-ai/dsh-base` verbatim. They are
...
**These ratings are not yet a ranking.** <!-- census:begin n-rated -->548<!-- census:end n-rated --> entries have a rating,
...
| top quartile | 137 | 1508–1508 | 1.0 |
| second quartile | 137 | 1500–1508 | 1.0 |
| third quartile | 137 | 1492–1500 | 1.0 |
| bottom quartile | 137 | 1492–1492 | 1.0 |
...
Over <!-- census:begin n-npm-checked -->3663<!-- census:end n-npm-checked --> packages that resolve on npm:

| State | Meaning | Count | Share |
| --- | --- | --- | --- |
<!-- census:begin npm-manifest -->
| `bundle-ok` | the published manifest declares `dsh.bundle` | 3220 | 87.9% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 180 | 4.9% |
| `package-missing` | the declared name no longer resolves on the registry | 24 | 0.7% |
| `unreadable` | the registry could not be read; not a statement about the package | 239 | 6.5% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->204<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.6%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->8398<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->14<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
**`inconclusive` is <!-- census:begin n-inconclusive -->124<!-- census:end n-inconclusive --> entries (<!-- census:begin pct-inconclusive -->1.5%<!-- census:end pct-inconclusive -->), and that is a
...
Coverage is <!-- census:begin n-reviewed -->1078<!-- census:end n-reviewed --> of <!-- census:begin n-catalogued -->8398<!-- census:end n-catalogued --> and grows with each scheduled run,
...
<!-- census:begin n-enumerated -->12378<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->10798<!-- census:end n-probed --> probed repositories, with the probe scripts included.
