...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->14336<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->14806<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->14806<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 11775 | 79.5% |
| `NO_DSH_FIELD` | 1250 | 8.4% |
| `NO_PACKAGE_JSON` | 889 | 6.0% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 740 | 5.0% |
| `VENDORED_HARNESS` | 47 | 0.3% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 41 | 0.3% |
...
<!-- census:begin n-enumerated -->14336<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->57<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->2904<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 6908 | 58.7% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 2904 | 24.7% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 384 | 3.3% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1062 | 9.0% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 517 | 4.4% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->7292<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->2904<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 5318 |
| `git-only` | absent from npm; installable from a Git specifier | 6279 |
...
**These ratings are not yet a ranking.** <!-- census:begin n-rated -->2064<!-- census:end n-rated --> entries have a rating,
averaging <!-- census:begin rating-matches-mean -->1.4<!-- census:end rating-matches-mean --> comparisons each with a maximum of
<!-- census:begin rating-matches-max -->18<!-- census:end rating-matches-max -->, and the spread is only
<!-- census:begin rating-spread -->1429 to 1603<!-- census:end rating-spread -->. Elo needs roughly 10 to 20 matches before a rating
...
therefore uneven, and that unevenness is the finding: <!-- census:begin n-onematch -->1539<!-- census:end n-onematch --> of the
<!-- census:begin n-rated -->2064<!-- census:end n-rated --> rated entries have exactly one comparison, so ratings sit on a few discrete
values and pile up at each end of the range.

<!-- census:begin rating-bands -->
| Band | Rating range | Entries | Mean matches |
| --- | --- | --- | --- |
| top band | 1508–1603 | 898 | 1.4 |
| second band | 1500–1504 | 195 | 1.6 |
| third band | 1492–1498 | 789 | 1.1 |
| bottom band | 1429–1489 | 182 | 3.0 |
...
Over <!-- census:begin n-npm-checked -->5318<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 4763 | 89.6% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 269 | 5.1% |
| `package-missing` | the declared name no longer resolves on the registry | 42 | 0.8% |
| `unreadable` | the registry could not be read; not a statement about the package | 244 | 4.6% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->311<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.8%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->11775<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->28<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
**`inconclusive` is <!-- census:begin n-inconclusive -->154<!-- census:end n-inconclusive --> entries (<!-- census:begin pct-inconclusive -->1.3%<!-- census:end pct-inconclusive -->), and that is a
...
The 162 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 150 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->14336<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->14806<!-- census:end n-probed --> probed repositories, with the probe scripts included.
