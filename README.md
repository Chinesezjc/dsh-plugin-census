...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->16004<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->16744<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->16744<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 13446 | 80.3% |
| `NO_DSH_FIELD` | 1352 | 8.1% |
| `NO_PACKAGE_JSON` | 971 | 5.8% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 799 | 4.8% |
...
<!-- census:begin n-enumerated -->16004<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
<!-- census:begin n-declared -->3547<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 7642 | 56.8% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 3547 | 26.4% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 457 | 3.4% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1179 | 8.8% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 621 | 4.6% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->8099<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->3547<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 6217 |
| `git-only` | absent from npm; installable from a Git specifier | 7047 |
...
Over <!-- census:begin n-npm-checked -->6217<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 5594 | 90.0% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 307 | 4.9% |
| `package-missing` | the declared name no longer resolves on the registry | 57 | 0.9% |
| `unreadable` | the registry could not be read; not a statement about the package | 259 | 4.2% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->364<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.9%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->13446<!-- census:end n-catalog-rows --> entries:
...
The 2804 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 2792 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->16004<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->16744<!-- census:end n-probed --> probed repositories, with the probe scripts included.
