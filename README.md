...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->12411<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->14184<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->14184<!-- census:end n-probed --> probed repositories, by verdict:

<!-- census:begin verdicts -->
| Verdict | Count | Share |
| --- | --- | --- |
| `CONTRACT_OK` | 11263 | 79.4% |
| `NO_DSH_FIELD` | 1196 | 8.4% |
| `NO_PACKAGE_JSON` | 866 | 6.1% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 718 | 5.1% |
| `VENDORED_HARNESS` | 47 | 0.3% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 38 | 0.3% |
...
<!-- census:begin n-enumerated -->12411<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->53<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->2734<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| --- | --- | --- | --- |
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 6665 | 59.2% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 2734 | 24.3% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 368 | 3.3% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 1014 | 9.0% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 482 | 4.3% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->7033<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->2734<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| --- | --- | --- |
| `published` | the declared name resolves on the npm registry | 5039 |
| `git-only` | absent from npm; installable from a Git specifier | 6047 |
...
**These ratings are not yet a ranking.** <!-- census:begin n-rated -->1136<!-- census:end n-rated --> entries have a rating,
...
| --- | --- | --- | --- |
| top quartile | 284 | 1508–1508 | 1.0 |
| second quartile | 284 | 1500–1508 | 1.0 |
| third quartile | 284 | 1492–1500 | 1.0 |
| bottom quartile | 284 | 1492–1492 | 1.0 |
...
Over <!-- census:begin n-npm-checked -->5039<!-- census:end n-npm-checked --> packages that resolve on npm:

<!-- census:begin npm-manifest -->
| State | Meaning | Count | Share |
| --- | --- | --- | --- |
| `bundle-ok` | the published manifest declares `dsh.bundle` | 4531 | 89.9% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 231 | 4.6% |
| `package-missing` | the declared name no longer resolves on the registry | 34 | 0.7% |
| `unreadable` | the registry could not be read; not a statement about the package | 243 | 4.8% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->265<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.3%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->11263<!-- census:end n-catalog-rows --> entries:
...
The 102 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 90 more in `data/decay.jsonl`.
...
<!-- census:begin n-enumerated -->12411<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->14184<!-- census:end n-probed --> probed repositories, with the probe scripts included.
