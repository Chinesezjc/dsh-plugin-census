...
The topic is enumerated in full rather than sampled: <!-- census:begin n-enumerated -->12538<!-- census:end n-enumerated --> unique
...
the <!-- census:begin n-probed -->11498<!-- census:end n-probed --> repositories probed so far:
...
All <!-- census:begin n-probed -->11498<!-- census:end n-probed --> probed repositories, by verdict:

| Verdict | Count | Share |
| --- | --- | --- |
<!-- census:begin verdicts -->
| `CONTRACT_OK` | 8995 | 78.2% |
| `NO_DSH_FIELD` | 1021 | 8.9% |
| `NO_PACKAGE_JSON` | 737 | 6.4% |
| `DSH_WITHOUT_BUNDLE_PATCH` | 629 | 5.5% |
| `VENDORED_HARNESS` | 41 | 0.4% |
| `PATCH_FILE_EMPTY_OR_INVALID` | 29 | 0.3% |
| `MALFORMED_PACKAGE_JSON` | 16 | 0.1% |
| `PATCH_FILE_MISSING` | 12 | 0.1% |
...
<!-- census:begin n-enumerated -->12538<!-- census:end n-enumerated --> enumerated repositories rather than a fresh sample each time.
...
packages. All <!-- census:begin n-vendored -->41<!-- census:end n-vendored --> have `fork: false`, so they are source copies that neither an
...
**Honest limit:** tiers 2 and 3 reject little in practice — <!-- census:begin n-tier23-fail -->41<!-- census:end n-tier23-fail --> of the
<!-- census:begin n-declared -->2064<!-- census:end n-declared --> repositories that declare a patch fail them.
...
| `high` | depends on `@deepseek-ai/dsh-client-*` (client) or `@deepseek-ai/dsh-host-*` and host-only packages (host) | 5583 | 62.1% |
| `declared` | the plugin's own `dsh.client` or `dsh.host` block declares the surface | 2064 | 22.9% |
| `medium` | depends on `@deepseek-ai/*`, but no dependency distinguishes client from host — surface `indeterminate` | 305 | 3.4% |
| `low` | no `@deepseek-ai/*` dependency; surface guessed from a name or description keyword | 721 | 8.0% |
| `none` | no dependency evidence and no keyword match — **not attributed at all** | 322 | 3.6% |
<!-- census:end surface-en -->

The <!-- census:begin n-dep-evidence -->5888<!-- census:end n-dep-evidence --> `high` and `medium` rows rest on an installed
dependency. A further <!-- census:begin n-declared -->2064<!-- census:end n-declared --> are `declared`: the plugin's own `dsh`
...
| `published` | the declared name resolves on the npm registry | 3980 |
| `git-only` | absent from npm; installable from a Git specifier | 4858 |
| `unpublishable-scope` | names itself under `@deepseek-ai/` from a repository outside that organisation | 155 |
| `unknown` | **the registry did not answer** — not a statement about the package | 2 |
<!-- census:end install-en -->

A further <!-- census:begin n-vendored -->41<!-- census:end n-vendored --> repositories carry `@deepseek-ai/dsh-base` verbatim. They are
...
**These ratings are not yet a ranking.** <!-- census:begin n-rated -->624<!-- census:end n-rated --> entries have a rating,
...
| top quartile | 156 | 1508–1508 | 1.0 |
| second quartile | 156 | 1500–1508 | 1.0 |
| third quartile | 156 | 1492–1500 | 1.0 |
| bottom quartile | 156 | 1492–1492 | 1.0 |
...
Over <!-- census:begin n-npm-checked -->3980<!-- census:end n-npm-checked --> packages that resolve on npm:

| State | Meaning | Count | Share |
| --- | --- | --- | --- |
<!-- census:begin npm-manifest -->
| `bundle-ok` | the published manifest declares `dsh.bundle` | 3587 | 90.1% |
| `bundle-missing` | **the published manifest declares no `dsh.bundle`** — DSH refuses it as a profile bundle | 196 | 4.9% |
| `package-missing` | the declared name no longer resolves on the registry | 26 | 0.7% |
| `unreadable` | the registry could not be read; not a statement about the package | 171 | 4.3% |
<!-- census:end npm-manifest -->

<!-- census:begin n-npm-broken -->222<!-- census:end n-npm-broken --> of them (<!-- census:begin pct-npm-broken -->5.6%<!-- census:end pct-npm-broken -->) **cannot be loaded by
...
Over all <!-- census:begin n-catalog-rows -->8995<!-- census:end n-catalog-rows --> entries:
...
the catalogued entries is <!-- census:begin max-age-days -->15<!-- census:end max-age-days --> days old, so a 30-day dormancy threshold
...
**`inconclusive` is <!-- census:begin n-inconclusive -->147<!-- census:end n-inconclusive --> entries (<!-- census:begin pct-inconclusive -->1.6%<!-- census:end pct-inconclusive -->), and that is a
...
The 44 entries flagged as decayed (`inconclusive` is not decay and is excluded):
...
...and 32 more in `data/decay.jsonl`.
...
Coverage is <!-- census:begin n-reviewed -->1078<!-- census:end n-reviewed --> of <!-- census:begin n-catalogued -->8995<!-- census:end n-catalogued --> and grows with each scheduled run,
...
<!-- census:begin n-enumerated -->12538<!-- census:end n-enumerated --> unique repositories, the whole topic rather than a sample.
...
- **Published distributions of the whole sample** rather than a curated
  ...
  <!-- census:begin n-probed -->11498<!-- census:end n-probed --> probed repositories, with the probe scripts included.
